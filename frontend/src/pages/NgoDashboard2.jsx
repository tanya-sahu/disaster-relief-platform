      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <section className="w-full border-r border-slate-800 bg-slate-950 flex flex-col md:w-5/12 overflow-hidden">
            {/* Priority Filter Box */}
            <div className="p-4 bg-slate-900/40 border-b border-slate-800/80 flex flex-col space-y-2">
              <label className="text-[11px] font-bold tracking-wider uppercase text-slate-400 flex items-center gap-1.5">
                <SlidersHorizontal className="w-3.5 h-3.5 text-indigo-400" />
                Filter Pipeline by Priority
              </label>
              <div className="relative">
                <select
                  value={priorityFilter}
                  onChange={(e) => setPriorityFilter(e.target.value)}
                  className="w-full bg-slate-950/80 text-sm border border-slate-800 rounded-xl px-4 py-2.5 text-slate-200 focus:outline-none focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/20 appearance-none transition"
                >
            ...
            {/* Scrollable Container wrapper for logs */}
          ...
          <section className="hidden md:flex md:w-7/12 flex-col bg-slate-900/20 overflow-y-auto">
            {selectedRequest ? (
              <div className="flex flex-col h-full">
                ...
                <div className="p-6 border-b border-slate-800 bg-slate-900/60 flex items-center justify-between">
                  <div>
                    ...
                    <h3 className="text-lg font-bold text-slate-100 flex items-center gap-2 capitalize">
                      <Package className="w-5 h-5 text-indigo-400" /> Operational Dispatch Metrics
                    </h3>
                  </div>
                  <div className="flex items-center space-x-2">
                    <span
                ...
                <div className="p-6 space-y-6 flex-1">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                      {selectedRequest.requestedItems?.map((item, idx) => {
                        // FIX: Clean, valid arithmetic calculation block
                        const rawRate = (item.fulfilledQuantity / item.requiredQuantity) * 100;
                        const completionRate = isNaN(rawRate) ? 0 : Math.min(100, Math.round(rawRate));
                                ...
                                <div
                                  className={`h-full transition-all duration-500 rounded-full ${
                                    itemDisplayStatus === "fulfilled"
                                      ...
                                      : ["partially-fulfilled", "partially fulfilled"].includes(itemDisplayStatus)
                  ...
                  <div className="text-[11px] text-slate-600 pt-4 border-t border-slate-800 space-y-1 font-mono">
                    <p>
                      SYSTEM INGEST TIME: {new Date(selectedRequest.createdAt).toLocaleString()}
                    </p>
              ...
              <div className="p-12 text-center text-sm text-slate-500 flex flex-col items-center justify-center h-full">
                ...
                <p>Select a request from the pipeline to review details.</p>
              </div>
            )}
          </section>
        </div>
